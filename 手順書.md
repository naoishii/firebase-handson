...
匿名とgoogleを有効化しましょう。余裕があればTwitter, Github, Facebookもやりたい。g
...
firebaseの設定をするだけのファイルを作ります。`src/firebase.js`gというファイルを作成し、先ほどコピーした設定を適用します。
```js
...
## gFirestore
FirebaseにはRealtime DatabaceとClowd Firestoreの2種類が用意されています。Firestoreはまだベータ版ですが、良さそうなので今日は使っていきたいと思います。
g
詳しい違いはこちら
...
### 初期設定
早速初期設定をやっていきましょう。
WebUIからDatabaceを選択し、データベースを選択します。今回はFirestoreです。
...
データを取ってくるところを関数化します。`modules/chat`にfetchCommentsアクションを作成します。(データの追加ではなく置き換えをするのでそのためのアクションとreducerを追加しています。)
```js
